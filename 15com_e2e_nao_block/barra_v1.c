...
int main(int argc, char **argv){
  ...
  // Processo 0 começa com calor na primeira célula real
  if (rank == 0){
    u[1] = 1000.0f;
  }
  ...
  for (int step = 0; step < STEPS; step++){
    // Troca de bordas com vizinhos
    if (rank > 0){
      MPI_Send(&u[1], 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD);
      MPI_Recv(&u[0], 1, MPI_FLOAT, rank - 1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    else{
    ...
    if (rank < size - 1){
      MPI_Send(&u[local_n], 1, MPI_FLOAT, rank + 1, 1, MPI_COMM_WORLD);
      MPI_Recv(&u[local_n + 1], 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    else{
      u[local_n + 1] = u[local_n]; // Borda direita fixa
    }

    // Atualiza as células reais com base nos vizinhos (difusão)
